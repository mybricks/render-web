<template>
  ...
  <!-- <div>{{ count }}</div> -->
  <MMain
    v-for="scene in scenes.filter(scene => scene)"
    :key="scene.key"
    :json="scene.json"
  ...
  />
  <MMain
    v-for="scene in popups.filter(scene => scene)"
    :key="scene.key"
    :json="scene.json"
    :options="scene.options"
    :style="scene.style"
...
const { ref, provide, inject, computed } = window.Vue
...
const setPageScenes = (pageScenes) => {
  data.value.pageScenes = pageScenes;
}
const setPopupIds = (popupIds) => {
  data.value.popupIds = popupIds;
}
...
const { scenesMap, scenesOperateInputsTodo, globalVarMap, env } = init()
...
//   return Math.random() + data.value.count
// })
...
const scenes = computed(() => {
  ...
  return data.value.pageScenes.map((json) => {
    const { id } = json
    const scene = scenesMap[id]
    if (scene.show) {
      // let className = scene.useEntryAnimation ? css.main : ''
      let className = scene.useEntryAnimation ? 'main' : '' // TODO
      let style = {}
      if (scene.main) {
        // 主场景
...
const popups = computed(() => {
  return data.value.popupIds.map((sceneId) => {
    const scene = scenesMap[sceneId]
