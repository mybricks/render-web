...
export default function Main({
  ...
  const { env, onError, logger, slot, getComDef } = useMemo(() => {
    ...
    const slot: any = json.slot;
    if (
      !env.canvas.isValid &&
      !options._isNestedRender &&
      debug &&
      from === "scene" &&
      slot &&
      slot.type !== "module" &&
      slot.showType !== "module"
    ) {
      /**
       * 1. 调试模式
       * 2. 场景（页面）
  ...
  const [context, refs, activeTriggerInput] = useMemo(() => {
    try {
      ...
      return [context, refs, activeTriggerInput] as any;
    } catch (ex: any) {
      console.error(ex);
      Notification.error(
        `导出的JSON.script执行异常: ${ex?.stack || ex?.message || ex?.toString?.()}`,
      );
