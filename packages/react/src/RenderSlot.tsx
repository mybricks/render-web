...
export default function RenderSlot({
  ...
  if (style.layout === "smart" && layoutTemplate) {
    ...
    return (
      <div data-isslot='1' onClick={onClick} className={`${calSlotClasses(slotStyle)}${root && className ? ` ${className}` : ''}`} style={{ overflow: root ? (showType === "module" ? "hidden" : "hidden auto") : null, ...calSlotStyles(slotStyle, true, root, slot.type === "module", options), ...propsStyle, display: 'flex', flexDirection: "column"}}>
