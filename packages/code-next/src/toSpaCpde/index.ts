...
export type UI = ToCodeResult[0]["ui"];
...
export interface BaseConfig extends ToSpaCodeConfig {
  ...
  getEventByDiagramId: (
    diagramId: string,
  ) => ReturnType<typeof toCode>[0]["event"][0];
  /** 获取事件 - 插槽 */
  ...
  getVarEvents: (params?: {
    comId: string;
    slotId: string;
  }) => ReturnType<typeof toCode>[0]["event"];
  /** 获取事件 - fx */
  getFxEvents: (params?: {
    comId: string;
    slotId: string;
